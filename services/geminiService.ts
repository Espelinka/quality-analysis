...
function getAiClient(): GoogleGenAI {
  if (!ai) {
    // This line, which uses process.env, is now only called when an analysis is requested,
    // not on initial page load. This prevents the "blank page" runtime error.
...
export const analyzeDefect = async (request: AnalysisRequest): Promise<AnalysisReport> => {
  try {
    ...
    // Check for specific Gemini errors if possible, otherwise throw a generic message
