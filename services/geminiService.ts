...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            // FIX: Updated error message to reflect the change to using `process.env.API_KEY`.
            throw new Error('Ошибка конфигурации: Ключ API не найден. Убедитесь, что переменная API_KEY правильно установлена в переменных окружения.');
        }
