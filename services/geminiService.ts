...
function getAiClient(): GoogleGenAI {
  if (!ai) {
    ...
    // This also resolves errors related to `import.meta.env` and missing Vite client types.
...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            // Fix: Changed error message to reference `API_KEY` instead of `VITE_API_KEY` per coding guidelines.
            throw new Error('Ошибка конфигурации: Ключ API не найден. Убедитесь, что в настройках Vercel установлена переменная окружения с именем API_KEY.');
        }
        if (error.message.includes("API key not valid")) {
            throw new Error('Ошибка конфигурации: Ваш API ключ недействителен. Пожалуйста, проверьте его значение.');
        }
        if (error.message.includes("Quota exceeded")) {
            throw new Error('Превышена квота использования API. Пожалуйста, проверьте ваш биллинг в Google AI Studio.');
        }
        // Handle 503 Service Unavailable / Overloaded error
        if (error.message.includes("overloaded") || error.message.includes("503") || error.message.includes("UNAVAILABLE")) {
            throw new Error('Сервер анализа временно перегружен. Пожалуйста, повторите попытку через несколько минут.');
        }
        if (error.message.includes("content has been blocked")) {
            throw new Error("Ответ от модели был заблокирован из-за настроек безопасности. Попробуйте другое изображение или запрос.");
        }
        if (error instanceof SyntaxError) {
            throw new Error("Ошибка обработки ответа. Сервис анализа вернул некорректные данные.");
        }
        if (error.message.includes("Ответ от модели пустой") || error.message.includes("некорректный формат")) {
             throw error;
        }
        // For other errors from the SDK or network, provide a more informative generic error.
