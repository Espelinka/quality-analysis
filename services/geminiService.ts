...
function getAiClient(): GoogleGenAI {
  if (!ai) {
    // Fix: Switched to `process.env.API_KEY` to align with the guidelines and fix TypeScript errors.
...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            // Fix: Updated error message to refer to `API_KEY` and be more generic.
            throw new Error('Ошибка конфигурации: Ключ API не найден. Убедитесь, что переменная окружения API_KEY установлена.');
        }
