...
function getAiClient(): GoogleGenAI {
  if (!ai) {
    // FIX: Use process.env.API_KEY as per the guidelines. The API key is expected to be in the environment.
...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            // FIX: Provide a more generic error message for missing API key as per guidelines.
            throw new Error('Ошибка конфигурации: Ключ API не найден.');
        }
