...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            throw new Error('Ошибка конфигурации: Ключ API не найден. Убедитесь, что переменная VITE_API_KEY правильно установлена в настройках Vercel.');
        }
        if (error.message.includes("API key not valid")) {
            throw new Error('Ошибка конфигурации: Ваш API ключ недействителен. Пожалуйста, проверьте его значение в Vercel.');
        }
