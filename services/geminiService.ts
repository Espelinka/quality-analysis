// Fix: Add triple-slash directive to include Vite's client types, resolving the TypeScript error for `import.meta.env`.
...
function getAiClient(): GoogleGenAI {
  if (!ai) {
    // CORRECT: Use `import.meta.env.VITE_API_KEY` for client-side Vite apps.
...
Уточняющий запрос: "${request.refinement || 'Нет уточнений.'}"`;
    ...
    if (error instanceof Error) {
        if (error.message === "API_KEY_MISSING") {
            // FIX: Updated error message to be more specific and helpful for Vercel deployment.
            throw new Error('Ошибка конфигурации: Ключ API не найден. Убедитесь, что в настройках Vercel установлена переменная окружения с именем VITE_API_KEY.');
        }
